...
- **Blog is a flat list of static pages**: `blog.html` is the index (`.post-list` of `.post-row` entries) linking to individual post pages named `blog-<slug>.html` at the repo root (e.g. `blog-mcp-itglue-documentation-server.html`). There's no CMS or data file. To add a post, run `python3 new-post.py "Title" --tags "A, B" --excerpt "…"` — it generates the post page (styled with the `.post-hero`/`.post-body` classes in `style.css`) and inserts the index entry at the top of `blog.html`, then the TODO paragraphs in the new file get replaced with content. The post-page template lives inside `new-post.py`; if the shared nav/footer changes, update the template there too.
